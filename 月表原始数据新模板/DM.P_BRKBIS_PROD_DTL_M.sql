create PROCEDURE DM.P_BRKBIS_PROD_DTL_M(IN @V_BIN_DATE INT)
...
BEGIN 
  ...
  *********************************************************************/
    ...
    --PART0 删除当月数据
    DELETE FROM DM.T_BRKBIS_PROD_DTL_M WHERE YEAR_MTH=@V_YEAR_MONTH;

    insert into DM.T_BRKBIS_PROD_DTL_M
    (
    YEAR_MTH
    ,ORG_NO
    ,BRH
	...
	select 
		...
		,t1.PROD_CD as 产品代码
        ,case when t_cp.PROD_NAME is null then '' else t_cp.PROD_NAME end  as 产品名称
        ,case when t_cp.MNG_PSN_NAME  is null then '' else t_cp.MNG_PSN_NAME end as 产品管理人
