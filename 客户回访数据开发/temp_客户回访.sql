    ...
    LEFT JOIN (SELECT FUND_ACCOUNT,PROD_CODE,SUM(BUSINESS_BALANCE) AS GMJE
               ...
               WHERE CURR_DATE BETWEEN 20180601 AND 20180630
     ...
     LEFT  JOIN DBA.T_DDW_D_JJ                    I  ON I.NIAN='2018' AND I.YUE='06' AND A.PROD_CODE=I.JJDM
    WHERE A.CURR_DATE BETWEEN 20180601 AND 20180630 AND A.DATE_CLEAR >= 20180701
    ...
    SELECT ZJZH||'-'||JJDM||'-'||WTRQ FROM query_dev.tmp_yybhf_fxpp 
    WHERE NIAN||YUE='2018'||'06');



    INSERT INTO query_dev.tmp_yybhf_fxpp(NIAN,YUE,WTRQ,YYBMC,ZJZH,KHBH,KHXM,XB,KHRQ,YWLX,WTJE,JJDM,JJMC,CPLX,CPFXDJ,KHFXDJ,YMCCSZ,BYGMJE,LX,JGBH)
    SELECT NIAN,YUE,WTRQ,YYBMC,ZJZH,KHBH,KHXM,XB,KHRQ,YWLX,WTJE,JJDM,JJMC,CPLX,CPFXDJ,KHFXDJ,YMCCSZ,BYGMJE,LX,JGBH
    FROM #T_ADD_SECUM;
    ...
    SELECT '2018'                                                    AS NIAN,                     
      ...
      LEFT JOIN (SELECT FUND_ACCOUNT,PROD_CODE,SUM(BUSINESS_BALANCE) AS GMJE
       ...
       LEFT  JOIN DBA.T_DDW_D_JJ                    I  ON I.NIAN='2018' AND I.YUE='06' AND A.PROD_CODE=I.JJDM
      WHERE A.CURR_DATE BETWEEN 20180601 AND 20180630
       AND A.ENTRUST_STATUS='8'                            
       AND A.DEAL_FLAG <> '4'                              
       AND A.BUSINESS_FLAG IN (44022, 44020)               
    ;
    ...
    SELECT 
           ...
           END                                                         AS LX,
           CONVERT(VARCHAR,A.BRANCH_NO)                                AS JGBH
      INTO #T_ADD_BANKPROD_TEMP
      FROM DBA.T_EDW_UF2_HIS_BANKMDELIVER    A
      LEFT JOIN DBA.T_ODS_TR_FUND_ACC_INFO   B ON CONVERT(VARCHAR, A.FUND_ACCOUNT) = B.FUND_ACC
      LEFT JOIN DBA.T_ODS_UF2_SYSDICTIONARY  C ON C.DICT_ENTRY = 2505 AND C.SUBENTRY = CONVERT(VARCHAR, A.CORP_RISK_LEVEL)
      LEFT JOIN (SELECT DISTINCT PROD_CODE,PRODRISK_LEVEL FROM DBA.T_EDW_UF2_PRODCODE  WHERE LOAD_DT BETWEEN  20180601 AND 20180630 ) D ON A.PROD_CODE=D.PROD_CODE
      LEFT JOIN DBA.T_ODS_UF2_SYSDICTIONARY  E ON E.DICT_ENTRY = 2505 AND E.SUBENTRY = CONVERT(VARCHAR, D.PRODRISK_LEVEL)
      ...
      LEFT JOIN (SELECT FUND_ACCOUNT,PROD_CODE,SUM(ENTRUST_BALANCE) AS GMJE
                ...
                WHERE T1.CURR_DATE BETWEEN 20180601 AND 20180630
